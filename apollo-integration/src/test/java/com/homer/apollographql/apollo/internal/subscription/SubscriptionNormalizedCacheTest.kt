...
import com.homer.apollographql.apollo.integration.subscription.NewRepoCommentSubscription
...
class SubscriptionNormalizedCacheTest {
  ...
  private lateinit var subscriptionCall: com.homer.apollographql.apollo.ApolloSubscriptionCall<NewRepoCommentSubscription.Data>
  ...
  fun `when no cache policy then response not cached`() {
    ...
    val uuid = (apolloClient.subscriptionManager as com.homer.apollographql.apollo.internal.subscription.RealSubscriptionManager).subscriptions.keys.first()
  ...
  fun `when network only cache policy then response is cached`() {
    ...
    val callback = SubscriptionManagerCallbackAdapter<NewRepoCommentSubscription.Data>()
    subscriptionCall.cachePolicy(
        com.homer.apollographql.apollo.ApolloSubscriptionCall.CachePolicy.NETWORK_ONLY).execute(callback)
  ...
  fun `when cache and network policy then first response from cache next one from network`() {
    ...
    val callback = SubscriptionManagerCallbackAdapter<NewRepoCommentSubscription.Data>()
    subscriptionCall.cachePolicy(
        com.homer.apollographql.apollo.ApolloSubscriptionCall.CachePolicy.CACHE_AND_NETWORK).execute(callback)
...
private class MockSubscriptionTransportFactory : com.homer.apollographql.apollo.subscription.SubscriptionTransport.Factory {
  var subscriptionTransport: MockSubscriptionTransport? = null
  var callback: com.homer.apollographql.apollo.subscription.SubscriptionTransport.Callback? = null

  override fun create(callback: com.homer.apollographql.apollo.subscription.SubscriptionTransport.Callback): com.homer.apollographql.apollo.subscription.SubscriptionTransport {
    this.callback = callback
    return MockSubscriptionTransport().also { subscriptionTransport = it }
  }
}

private class MockSubscriptionTransport : com.homer.apollographql.apollo.subscription.SubscriptionTransport {
...
private class SubscriptionManagerCallbackAdapter<T> : com.homer.apollographql.apollo.ApolloSubscriptionCall.Callback<T> {
