 ...
 *
 * <p>A non-null {@code Optional<T>} reference can be used as a replacement for a nullable {@code T} reference. It
 ...
 *
 * <p><b>Comparison to {@code java.util.Optional} (JDK 8 and higher):</b> A new {@code Optional} class was added for
 * Java 8. The two classes are extremely similar, but incompatible (they cannot share a common supertype). <i>All</i>
 * known differences are listed either here or with the relevant methods below.
 *
 * <ul> <li>This class is serializable; {@code java.util.Optional} is not. <li>{@code java.util.Optional} has the
 * additional methods {@code ifPresent}, {@code filter}, {@code flatMap}, and {@code orElseThrow}. <li>{@code java.util}
 * offers the primitive-specialized versions {@code OptionalInt}, {@code OptionalLong} and {@code OptionalDouble}, the
 * use of which is recommended; Guava does not have these. </ul>
 *
 ...
 *            safe to cast an {@code Optional<T>} to {@code Optional<S>} for any supertype {@code S} of {@code T}.
...
public abstract class Optional<T> implements Serializable {
   ...
   *
   * <p><b>Comparison to {@code java.util.Optional}:</b> this method is equivalent to Java 8's {@code Optional.empty}.
   */
   ...
   * {@link #orNull} instead.
   *
   * <p><b>Comparison to {@code java.util.Optional}:</b> when the value is absent, this method throws {@link
   ...
   *                               this <i>specific</i> exception type (over the more general {@link RuntimeException})
   *                               is discouraged
   */
   ...
   *
   * <p>Note about generics: The signature {@code public T or(T defaultValue)} is overly restrictive. However, the ideal
   * signature, {@code public <S super T> S or(S)}, is not legal Java. As a result, some sensible operations involving
   ...
   *
   * <p>As a workaround, it is always safe to cast an {@code Optional<? extends T>} to {@code Optional<T>}. Casting
   * either of the above example {@code Optional} instances to {@code Optional<Number>} (where {@code Number} is the
   ...
   *
   * <p><b>Comparison to {@code java.util.Optional}:</b> this method is similar to Java 8's {@code Optional.orElse}, but
   ...
   *
   * <p><b>Comparison to {@code java.util.Optional}:</b> this method has no equivalent in Java 8's {@code Optional}
   ...
   *
   * <p><b>Comparison to {@code java.util.Optional}:</b> this class leaves the specific choice of hash code unspecified,
   ...
   *
   * <p><b>Comparison to {@code java.util.Optional}:</b> this class leaves the specific string representation
