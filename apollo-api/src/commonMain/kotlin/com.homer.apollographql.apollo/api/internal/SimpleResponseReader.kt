...
import com.homer.apollographql.apollo.api.internal.ResponseReader.ListReader
import com.homer.apollographql.apollo.api.internal.ResponseReader.ObjectReader
...
class SimpleResponseReader private constructor(
  ...
  override fun <T : Any> readObject(field: ResponseField, objectReader: ObjectReader<T>): T? {
    if (shouldSkip(field)) {
      return null
    }
  ...
  override fun <T : Any> readList(field: ResponseField, listReader: ListReader<T>): List<T?>? {
    if (shouldSkip(field)) {
      return null
    }
  ...
  override fun <T : Any> readFragment(field: ResponseField, objectReader: ObjectReader<T>): T? {
    if (shouldSkip(field)) {
      return null
    }
  ...
  private inner class ListItemReader internal constructor(
    ...
    @Suppress("UNCHECKED_CAST")
    override fun <T : Any> readObject(objectReader: ObjectReader<T>): T {
    ...
    @Suppress("UNCHECKED_CAST")
    override fun <T : Any> readList(listReader: ListReader<T>): List<T?> {
