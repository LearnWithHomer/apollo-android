...
import com.homer.apollographql.apollo.api.internal.json.Utils.writeToJson
import com.homer.apollographql.apollo.api.internal.ResponseWriter.ListItemWriter
import com.homer.apollographql.apollo.api.internal.ResponseWriter.ListWriter
import com.homer.apollographql.apollo.api.internal.json.use
import okio.Buffer
...
class SimpleResponseWriter(private val scalarTypeAdapters: ScalarTypeAdapters) : ResponseWriter {
  ...
  override fun <T> writeList(field: ResponseField, values: List<T>?, listWriter: ListWriter<T>) {
    if (values == null) {
      data[field.responseName] = null
    } else {
  ...
  private class CustomListItemWriter internal constructor(private val scalarTypeAdapters: ScalarTypeAdapters) :
      ListItemWriter {
    ...
    override fun <T> writeList(items: List<T>?, listWriter: ListWriter<T>) {
      if (items == null) {
        data.add(null)
      } else {
        val listItemWriter = CustomListItemWriter(scalarTypeAdapters)
