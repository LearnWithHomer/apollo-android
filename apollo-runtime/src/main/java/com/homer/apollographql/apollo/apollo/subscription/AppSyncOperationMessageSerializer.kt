...
import com.homer.apollographql.apollo.apollo.subscription.ApolloOperationMessageSerializer.writePayloadContentsTo
...
import com.homer.apollographql.apollo.apollo.subscription.OperationClientMessage.Start.Companion
...
class AppSyncOperationMessageSerializer(
  ...
  override fun writeClientMessage(message: OperationClientMessage, sink: BufferedSink) {
    when (message) {
      is Start -> JsonWriter.of(sink).use { message.writeTo(it) }
      is Init,
      is Stop,
      is Terminate -> ApolloOperationMessageSerializer.writeClientMessage(message, sink)
    }
  }
  ...
  private fun Start.writeTo(writer: JsonWriter) {
    writer.writeObject {
      name(ApolloOperationMessageSerializer.JSON_KEY_ID).value(subscriptionId)
      name(ApolloOperationMessageSerializer.JSON_KEY_TYPE).value(Start.TYPE)
  ...
  private fun Start.serializePayload(): String =
      Buffer().use { buffer ->
