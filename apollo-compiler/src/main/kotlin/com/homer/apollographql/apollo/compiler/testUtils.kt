  ...
  return toBuilder()
      .addMethod(MethodSpec.constructorBuilder()
          ...
          .addCode(fieldSpecs
              .filter { !it.modifiers.contains(Modifier.STATIC) }
              .map {
                if (it.type.isOptional() && nullableValueGenerationType != ANNOTATED) {
                  val factory = when (nullableValueGenerationType) {
                    GUAVA_OPTIONAL -> ClassNames.GUAVA_OPTIONAL to "fromNullable"
                    JAVA_OPTIONAL -> ClassNames.JAVA_OPTIONAL to "ofNullable"
...
fun TypeSpec.withToStringImplementation(): TypeSpec {
  ...
  return toBuilder()
      .addField(FieldSpec.builder(
          ClassNames.STRING, Util.MEMOIZED_TO_STRING_VAR, Modifier.PRIVATE, Modifier.VOLATILE,
          Modifier.TRANSIENT)
          .build())
...
object Util {
  ...
  val SCALAR_TYPES = listOf(
      ClassNames.STRING, TypeName.INT, TypeName.INT.box(), TypeName.LONG,
