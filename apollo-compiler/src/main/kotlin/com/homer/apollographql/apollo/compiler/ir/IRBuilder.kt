...
import com.homer.apollographql.apollo.compiler.ir.TypeDeclaration.Companion.KIND_ENUM
import com.homer.apollographql.apollo.compiler.ir.TypeDeclaration.Companion.KIND_INPUT_OBJECT_TYPE
import com.homer.apollographql.apollo.compiler.ir.TypeDeclaration.Companion.KIND_SCALAR_TYPE
import com.homer.apollographql.apollo.compiler.parser.error.DocumentParseException
import com.homer.apollographql.apollo.compiler.ir.ScalarType.BOOLEAN
import com.homer.apollographql.apollo.compiler.ir.ScalarType.FLOAT
import com.homer.apollographql.apollo.compiler.ir.ScalarType.ID
import com.homer.apollographql.apollo.compiler.ir.ScalarType.INT
import com.homer.apollographql.apollo.compiler.parser.error.ParseException
import com.homer.apollographql.apollo.compiler.parser.graphql.DocumentParseResult
import com.homer.apollographql.apollo.compiler.parser.graphql.checkMultipleFragmentDefinitions
import com.homer.apollographql.apollo.compiler.parser.graphql.checkMultipleOperationDefinitions
import com.homer.apollographql.apollo.compiler.parser.graphql.validateArguments
import com.homer.apollographql.apollo.compiler.parser.introspection.IntrospectionSchema
import com.homer.apollographql.apollo.compiler.parser.introspection.IntrospectionSchema.Kind.ENUM
import com.homer.apollographql.apollo.compiler.parser.introspection.IntrospectionSchema.Kind.INPUT_OBJECT
import com.homer.apollographql.apollo.compiler.parser.introspection.IntrospectionSchema.Kind.SCALAR
...
import com.homer.apollographql.apollo.compiler.parser.introspection.asGraphQLType
import com.homer.apollographql.apollo.compiler.parser.introspection.possibleTypes
import com.homer.apollographql.apollo.compiler.parser.introspection.rootTypeForOperationType
...
class IRBuilder(private val schema: IntrospectionSchema,
  ...
  fun build(documentParseResult: DocumentParseResult): CodeGenerationIR {
    ...
    val scalarsToGenerate = when {
      isRootCompilationUnit -> schema.types.values.filter { it is IntrospectionSchema.Type.Scalar }.map { it.name } + ID.name
      incomingMetadata != null -> emptyList()
      else -> typeDeclarations.filter { it.kind == KIND_SCALAR_TYPE }.map { it.name } + ID.name
    }.filter {
  ...
  private fun Set<String>.usedTypeDeclarations(): List<TypeDeclaration> {
    return usedSchemaTypes().map { type ->
      TypeDeclaration(
          ...
          values = (type as? Enum)?.enumValues?.map { value ->
          ...
          fields = (type as? InputObject)?.inputFields?.map { field ->
    ...
    while (inputTypesToVisit.isNotEmpty()) {
      ...
      val (nestedScalarTypes, nestedInputTypes) = inputType
          ...
          .let { (scalarTypes, inputTypes) ->
            @Suppress("UNCHECKED_CAST")
            scalarTypes.filter { ScalarType.forName(
                it.name
            ) == null } to (inputTypes as List<IntrospectionSchema.Type.InputObject>)
          }
