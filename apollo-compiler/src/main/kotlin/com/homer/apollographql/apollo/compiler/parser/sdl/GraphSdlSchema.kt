...
import com.homer.apollographql.apollo.compiler.parser.sdl.GraphSDLSchemaParser.parse
    ...
    private fun GraphSdlSchema.validate() {
      typeDefinitions.values
        ...
        o.interfaces.forEach { i ->
          if (typeDefinitions.get(i.typeName) !is Interface) {
...
fun GraphSdlSchema.toIntrospectionSchema(): IntrospectionSchema {
  return IntrospectionSchema(
      ...
      types = typeDefinitions.mapValues { (_, typeDefinition) ->
        when (typeDefinition) {
          is Enum -> typeDefinition.toIntrospectionType()
          is Object -> typeDefinition.toIntrospectionType(schema = this)
          is Interface -> typeDefinition.toIntrospectionType(schema = this)
          is InputObject -> typeDefinition.toIntrospectionType(schema = this)
          is Union -> typeDefinition.toIntrospectionType(schema = this)
          is Scalar -> typeDefinition.toIntrospectionType()
        }
      }
  )
}
...
private fun Enum.toIntrospectionType(): IntrospectionSchema.Type.Enum {
...
private fun Object.toIntrospectionType(schema: GraphSdlSchema): IntrospectionSchema.Type.Object {
...
private fun Interface.toIntrospectionType(schema: GraphSdlSchema): IntrospectionSchema.Type.Interface {
...
private fun Union.toIntrospectionType(schema: GraphSdlSchema): IntrospectionSchema.Type.Union {
...
private fun InputObject.toIntrospectionType(schema: GraphSdlSchema): IntrospectionSchema.Type.InputObject {
...
private fun Field.toIntrospectionType(schema: GraphSdlSchema): IntrospectionSchema.Field {
...
private fun InputField.toIntrospectionType(schema: GraphSdlSchema): IntrospectionSchema.InputField {
  val deprecated = directives.findDeprecatedDirective()
...
private fun Scalar.toIntrospectionType(): IntrospectionSchema.Type.Scalar {
...
private fun TypeRef.toIntrospectionType(schema: GraphSdlSchema): IntrospectionSchema.TypeRef {
  return when (this) {
    is Named -> {
    ...
    is NonNull -> IntrospectionSchema.TypeRef(
        name = null,
        kind = IntrospectionSchema.Kind.NON_NULL,
        ofType = typeRef.toIntrospectionType(schema)
    )

    is TypeRef.List -> IntrospectionSchema.TypeRef(
        name = null,
...
private fun TypeDefinition.toIntrospectionType(): IntrospectionSchema.Kind {
  return when (this) {
    is Enum -> IntrospectionSchema.Kind.ENUM
    is Object -> IntrospectionSchema.Kind.OBJECT
    is Interface -> IntrospectionSchema.Kind.INTERFACE
    is InputObject -> IntrospectionSchema.Kind.INPUT_OBJECT
    is Union -> IntrospectionSchema.Kind.UNION
    is Scalar -> IntrospectionSchema.Kind.SCALAR
  }
}

private fun Argument.toIntrospectionType(schema: GraphSdlSchema): IntrospectionSchema.Field.Argument {
  val deprecated = directives.findDeprecatedDirective()
...
private fun List<Directive>.findDeprecatedDirective(): DeprecateDirective? {
...
private fun Interface.possibleTypes(schema: GraphSdlSchema): List<IntrospectionSchema.TypeRef> {
  return schema.typeDefinitions.values
      .filter { typeDefinition ->
        typeDefinition is Object && typeDefinition.interfaces.firstOrNull { interfaceTypeRef ->
