...
import com.homer.apollographql.apollo.compiler.fromJson
import com.homer.apollographql.apollo.compiler.parser.introspection.IntrospectionQuery.MutationType
import com.homer.apollographql.apollo.compiler.parser.introspection.IntrospectionQuery.QueryType
import com.homer.apollographql.apollo.compiler.parser.introspection.IntrospectionQuery.Schema
import com.homer.apollographql.apollo.compiler.parser.introspection.IntrospectionQuery.SubscriptionType
import com.homer.apollographql.apollo.compiler.parser.introspection.IntrospectionQuery.Wrapper
...
import com.homer.apollographql.apollo.compiler.parser.introspection.IntrospectionSchema.Kind.INTERFACE
...
import com.homer.apollographql.apollo.compiler.parser.introspection.IntrospectionSchema.Kind.UNION
...
data class IntrospectionSchema(
    ...
    val types: Map<String, Type>) : Map<String, Type> by types {
    ...
    return copy(
        types = types.mapValues {
          when (val type = it.value) {
            is Object -> type.validate()
            is Enum -> type.validate()
            is InputObject -> type.validate()
            else -> type
          }
        }
    )
  ...
  private fun Object.validate() = copy(fields = fields?.map { it.validate() })

  private fun Enum.validate() = copy(enumValues = enumValues.map { it.validate() })

  private fun InputObject.validate() = copy(inputFields = inputFields.map { it.validate() })
  ...
  private fun Value.validate(): Value {
    return when {
      isDeprecated && deprecationReason == null -> {
  ...
  private fun Argument.validate(): Argument {
    return when {
      isDeprecated && deprecationReason == null -> {
  ...
  companion object {
    ...
    operator fun invoke(inputStream: InputStream, origin: String = ""): IntrospectionSchema {
      ...
      return try {
        jsonReader.locateSchemaRootNode().fromJson<Schema>().toIntrospectionSchema()
      } catch (e: Exception) {
        throw RuntimeException("Failed to parse GraphQL schema introspection query $origin", e)
      } finally {
        jsonReader.close()
    ...
    fun IntrospectionSchema.wrap(): Wrapper {
      return Wrapper(
          __schema = Schema(
              queryType = this.queryType.let { QueryType(it) },
              mutationType = this.mutationType?.let { MutationType(it) },
              subscriptionType = this.subscriptionType?.let { SubscriptionType(it) },
