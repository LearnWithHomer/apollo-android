package com.homer.apollographql.apollo.network.ws
...
import com.homer.apollographql.apollo.network.ws.ApolloGraphQLClientMessage.Init
import com.homer.apollographql.apollo.network.ws.ApolloGraphQLClientMessage.Start
import com.homer.apollographql.apollo.network.ws.ApolloGraphQLClientMessage.Stop
import com.homer.apollographql.apollo.network.ws.ApolloGraphQLServerMessage.Companion.parse
import com.homer.apollographql.apollo.network.ws.ApolloGraphQLServerMessage.Complete
import com.homer.apollographql.apollo.network.ws.ApolloGraphQLServerMessage.ConnectionAcknowledge
import com.homer.apollographql.apollo.network.ws.ApolloGraphQLServerMessage.ConnectionKeepAlive
import com.homer.apollographql.apollo.network.ws.ApolloGraphQLServerMessage.Data
import com.homer.apollographql.apollo.network.ws.ApolloGraphQLServerMessage.Error
...
class ApolloWebSocketNetworkTransport(
  ...
  override fun <D : Operation.Data> execute(request: ApolloRequest<D>, executionContext: ExecutionContext): Flow<ApolloResponse<D>> {
    ...
    return getServerConnection(dispatcherContext).flatMapLatest { serverConnection ->
      serverConnection
          .subscribe()
          .filter { message -> message !is ConnectionAcknowledge }
          .takeWhile { message -> message !is Complete || message.id != request.requestUuid.toString() }
  ...
  private suspend fun openServerConnection(dispatcherContext: ApolloCoroutineDispatcherContext): GraphQLWebsocketConnection {
    return try {
      withTimeout(connectionAcknowledgeTimeoutMs) {
        ...
        webSocketConnection.send(Init(connectionParams).serialize())
        while (webSocketConnection.receive().parse() !is ConnectionAcknowledge) {
  ...
  private class GraphQLWebsocketConnection(
    ...
    fun subscribe(): Flow<ApolloGraphQLServerMessage> {
      return messageChannel.openSubscription()
          ...
          .map { data -> data.parse() }
          .onEach { message ->
            if (message is ConnectionKeepAlive) onConnectionKeepAlive()
          }
    }
