...
At any time, if you want to view a completed version of the project, checkout the `master` branch.
...
This tutorial uses `2.2.0` because it is the latest version at the time of writing. Feel free to use a more recent version if one is available.

## Apply the plugin

Apply the Apollo plugin in `app/build.gradle.kts`. There are two `build.gradle.kts` in the project. Make sure to use the one in the `app` directory this time. The Apollo plugin ID is `com.homer.apollographql.apollo`.

```kotlin:title=app/build.gradle.kts
plugins {
    id("com.android.application")
    // ...
    id("com.homer.apollographql.apollo").version("2.2.0")
}
```
...
dependencies {
    ...
    // ...
    implementation 'com.homer.apollographql.apollo:apollo-runtime:2.2.0'
    implementation 'com.homer.apollographql.apollo:apollo-coroutines-support:2.2.0'
    // ...
}
```
