...
Other possibilities are `CACHE_ONLY`, `NETWORK_ONLY`, `CACHE_AND_NETWORK_ONLY` and `NETWORK_FIST`. See to the [`ResponseFetchers` class](https://github.com/apollographql/apollo-android/blob/main/apollo-runtime/src/main/java/com/apollographql/apollo/fetcher/ApolloResponseFetchers.java) for more details.
