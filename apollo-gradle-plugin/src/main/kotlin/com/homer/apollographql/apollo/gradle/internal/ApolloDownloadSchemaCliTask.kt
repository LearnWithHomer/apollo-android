...
abstract class ApolloDownloadSchemaCliTask : DefaultTask() {
  ...
  @get:Optional
  @get:Input
  @get:Option(option = "key", description = "The Apollo API key. See https://www.homer.apollographql.com/docs/studio/api-keys/ for more information on how to get your API key.")
  ...
  private fun Property<String>.orProperty(name: String) = orElse(project.provider {
    (project.findProperty("com.homer.apollographql.apollo.$name") as? String)?.also {
      logger.lifecycle("Using the com.homer.apollographql.apollo.$name property is deprecated. Use --$name instead.")
    }
  }).orNull
  ...
  fun taskAction() {
    ...
    val headersProp = project.findProperty("com.homer.apollographql.apollo.headers") as? String
    val headers = when {
      headersProp != null -> {
        logger.lifecycle("Using the com.homer.apollographql.apollo.headers property is deprecated. Use --header instead.")
        ApolloPlugin.toMap(headersProp)
      }
      else -> header.toMap()
    }

    val queryParamsProp = project.findProperty("com.homer.apollographql.apollo.query_params") as? String
    if (queryParamsProp != null) {
      logger.lifecycle("Using the com.homer.apollographql.apollo.query_params property is deprecated. Add parameters to the endpoint instead.")
